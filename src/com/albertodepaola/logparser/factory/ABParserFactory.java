...
public class ABParserFactory implements AbstractParserFactory<ABParserResult> {

	@Override
	public Parser<ABParserResult> createParser(Map<String, String> argumentsMap) {
		return createParser(argumentsMap, Configuration.getConfiguration().getLogConfiguration());
	}

	public static Parser<ABParserResult> createParser(Map<String, String> argumentsMap, Map<String, Integer> configMap) {
		ABParser.ParserBuilder parserBuilder = null;
		// TODO use reflection to simply iterate over map to initialize parser
		try {
			if (argumentsMap.containsKey("accesslog")) {
				parserBuilder = new ABParser.ParserBuilder(argumentsMap.get("accesslog"));
			} else {
...
//		Parser<ParserResult<WHParserResult>> parser = new WHParser();
		Parser<ABParserResult> parser = parserBuilder.build();
		return parser;
	}
